...
Read the issue body and decide: is this a **log entry**, a **workout log**, a **reflection**, or a **general note**?
...
→ log.csv: `2026-02-26,deep_work,1,90min - API routes`
...
**Deep work sessions** always get BOTH a log.csv signal (`deep_work,1,duration - topic`) AND a reflections.csv entry if there's any lesson/change content.
...
4. If it's a reflection: append to `reflections.csv` (and `log.csv` signal if applicable)
5. If it's a general note: append a `note` row to `log.csv` in format `date,note,<text>,`
6. If it contains BOTH log-worthy data AND reflections/notes, do both
7. After processing, comment on the GitHub issue with what you did
8. Close the issue
...
**Before writing the notification:** Read log.csv (and workouts.csv if gym-related) to compute context — streaks, trends, frequencies. Use this data to enrich the notification.
...
Read log.csv for gym frequency this month.
```
...
Read last 7 days of that metric from log.csv.
```
...
Read today's log.csv entries for calories and food-related notes. Read last 7 days of ate_clean.
```
...
Compute consecutive clean days from log.csv going backwards from today.
```
...
Read all previous trigger entries from log.csv. Count frequency. Check what followed previous similar triggers.
```
...
Read log.csv for deep_work entries today and this week.
```
✓ 90min on API routes. This week: 5 sessions.
```
- Mirror topic and duration from log.csv notes
...
- If you can't determine intent, default to a `note` row in log.csv
