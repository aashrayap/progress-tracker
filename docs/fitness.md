# Fitness

## Stats
- Age: 30
- Height: 6'0" (183 cm)
- Current: 239 lbs (108 kg)
- Goal: 200 lbs by Jun 1

## BMR (Mifflin-St Jeor)

```
(10 × 109kg) + (6.25 × 183cm) - (5 × 30) + 5 = 2,089 cal/day
```

## TDEE

| Activity | Multiplier | TDEE |
|----------|------------|------|
| Sedentary | 1.2 | 2,507 |
| Light (8k steps) | 1.375 | 2,872 |
| Moderate | 1.55 | 3,238 |

## Deficit at 1,900 cal/day

| Activity | Deficit/day | Deficit/week | lbs/week |
|----------|-------------|--------------|----------|
| Sedentary | -607 | -4,249 | 1.2 |
| Light | -972 | -6,804 | **1.9** |
| Moderate | -1,338 | -9,366 | 2.7 |

## Target
- 40 lbs ÷ 21 weeks = 1.9 lbs/week
- Need: 6,650 cal/week deficit
- **1,900 cal + 8k steps = on track**

## Cardio

Daily dog walks — 20-45 min, conversational pace.
Counts toward NEAT (non-exercise activity thermogenesis), not a formal workout.
